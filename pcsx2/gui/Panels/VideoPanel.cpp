	...
	: BaseApplicableConfigPanel_SpecificConfig( parent )
{
	...
	*this += m_check_LimiterDisable | StdExpand();
	...
	s_spins += Label(_("Slow Motion Adjust:") + wxString(" ") + fmt::format("({})", wxGetApp().GlobalAccels->findKeycodeWithCommandId("Framelimiter_SlomoToggle").toTitleizedString().ToStdString())) | StdExpand();
	s_spins += 5;
	...
	s_spins += Label(_("Turbo Adjust:") + wxString(" ") + fmt::format("({})", wxGetApp().GlobalAccels->findKeycodeWithCommandId("Framelimiter_TurboToggle").toTitleizedString().ToStdString())) | StdExpand();
	s_spins += 5;
	...
	: BaseApplicableConfigPanel_SpecificConfig( parent )
{
	const RadioPanelItem FrameskipOptions[] =
	{
		...
		//  Implement custom hotkeys (Tab) with translatable string intact + not blank in GUI.  
		RadioPanelItem(
			_("Skip only on Turbo, to enable press") + fmt::format("{} ({})", " ", wxGetApp().GlobalAccels->findKeycodeWithCommandId("Framelimiter_TurboToggle").toTitleizedString().ToStdString())
		),
		//  Implement custom hotkeys (Shift + F4) with translatable string intact + not blank in GUI.  
		RadioPanelItem(
			_("Constant skipping") + fmt::format("{} ({})", " ", wxGetApp().GlobalAccels->findKeycodeWithCommandId("Frameskip_Toggle").toTitleizedString().ToStdString()),
			wxEmptyString,
